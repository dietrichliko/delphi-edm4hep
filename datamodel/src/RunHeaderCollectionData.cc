...
RunHeaderCollectionData::RunHeaderCollectionData() :
  m_data(new RunHeaderDataContainer()) {
}

RunHeaderCollectionData::RunHeaderCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<delphi::RunHeaderData>());
...
void RunHeaderCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers RunHeaderCollectionData::getCollectionBuffers(bool isSubsetColl) {
return {
...
void RunHeaderCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) { pointer->clear(); }
...
void RunHeaderCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new RunHeaderObj({index, collectionID}, data);
...
bool RunHeaderCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider, bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      delphi::RunHeaderObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<delphi::RunHeaderCollection*>(coll);
...
void RunHeaderCollectionData::makeSubsetCollection() {
