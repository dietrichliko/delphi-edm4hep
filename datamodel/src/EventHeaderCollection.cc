...
void EventHeaderCollection::push_back(MutableEventHeader object) {
...
void EventHeaderCollection::push_back(EventHeader object) {
  if (!m_isSubsetColl) {
...
std::vector<std::uint32_t> EventHeaderCollection::eventNumber(const size_t nElem) const {
...
std::vector<std::uint32_t> EventHeaderCollection::runNumber(const size_t nElem) const {
  std::vector<std::uint32_t> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
...
std::vector<std::uint32_t> EventHeaderCollection::fileNumber(const size_t nElem) const {
  std::vector<std::uint32_t> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
...
std::vector<delphi::TimeStamp> EventHeaderCollection::timeStamp(const size_t nElem) const {
  std::vector<delphi::TimeStamp> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
...
std::vector<std::uint32_t> EventHeaderCollection::delphiDate(const size_t nElem) const {
  std::vector<std::uint32_t> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.delphiDate);
  }
  return tmp;
}

std::vector<std::uint32_t> EventHeaderCollection::delphiTime(const size_t nElem) const {
  std::vector<std::uint32_t> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.delphiTime);
  }
  return tmp;
}
...
podio::CollectionReadBuffers createBuffers(bool isSubset) {
  ...
  readBuffers.type = "delphi::EventHeaderCollection";
  ...
  readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
    ...
    if (buffers.data) {
      buffers.data = podio::CollectionWriteBuffers::asVector<delphi::EventHeaderData>(buffers.data);
    }
  };
  ...
  readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
    if (buffers.data) {
      ...
      // we can delete
      delete static_cast<delphi::EventHeaderDataContainer*>(buffers.data);
...
bool registerCollection() {
  const static auto reg = []() {
    ...
    factory.registerCreationFunc("delphi::EventHeaderCollection", delphi_edm4hep::meta::schemaVersion, createBuffers);
...
std::ostream& operator<<(std::ostream& o, const EventHeaderCollection& v) {
  ...
  o << "          id: eventNumber:   runNumber:  fileNumber:timeStamp [ unixTime]:  delphiDate:  delphiTime:" << '\n';

  for (const auto&& el : v) {
    o << std::scientific << std::showpos << std::setw(12) << el.id() << " "
      << std::setw(12) << el.getEventNumber() << " "
      << std::setw(12) << el.getRunNumber() << " "
      << std::setw(12) << el.getFileNumber() << " "
      << std::setw(12) << el.getTimeStamp() << " "
      << std::setw(12) << el.getDelphiDate() << " "
      << std::setw(12) << el.getDelphiTime() << " "
      << std::endl;
